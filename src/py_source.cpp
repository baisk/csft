...
#if USE_PYTHON_CASE_SENSIVE_ATTR
			pArgs  = Py_BuildValue("(sO)", tAttr.m_sNameExactly.cstr(), m_Hit_collector);
#else
			pArgs  = Py_BuildValue("(sO)", tAttr.m_sName.cstr(), m_Hit_collector);
#endif
...
BYTE* CSphSource_Python::GetField ( int iFieldIndex)
{
...
#ifdef _WIN32
		snprintf(buf,sizeof(buf), "%I64u\0", dVal);
#else
		snprintf(buf,sizeof(buf), "%llu\0", dVal);
#endif
		ptr = (BYTE*)strdup(buf);
	}
	...
	if(ptr == NULL && item && Py_None!=item && PyInt_Check(item)) {
		...
		char buf[128];
		memset(buf,0,sizeof(buf));
		snprintf(buf,sizeof(buf), "%d\0", dVal);
		ptr = (BYTE*)strdup(buf);
	}
...
void CSphSource_Python::BuildHits ( CSphString & sError, bool bSkipEndMarker)
{
	//fixme: restore buildhit call later.
	assert ( m_pTokenizer );
	...
	if (!m_pInstance)
	{
		...
		if(m_baseFields.GetLength() !=0)
		{
			ARRAY_FOREACH ( i, m_baseFields )
			{
				int j = this->m_tSchema.GetFieldIndex (m_baseFields[i].cstr());
				if(j == -1) {
					//must die, else might cause infinit loop
					sphDie( "Can Not found field named %s.\n" , m_baseFields[i].cstr());
				}
				else{
					m_tState.m_iField = j;
					m_tState.m_iEndField = j+1;
					BuildHits_Python( sError, bSkipEndMarker);
				}
			}
		}		
	}
...
void CSphSource_Python::BuildHits_Python ( CSphString & sError, bool bSkipEndMarker )
{
	//check have python layer?
	if(m_bHaveCheckBuildHit && (m_pInstance_BuildHit == NULL || m_pInstance_BuildHit == Py_None) )
	{
		return CSphSource_Document::BuildHits( sError, bSkipEndMarker);
	}
	...
	if(m_pInstance_BuildHit == NULL)
	{
		...
		if (m_pInstance)
		{
			m_pInstance_BuildHit = PyObject_GetAttrString(m_pInstance, "BuildHits"); // +1
		}

		m_bHaveCheckBuildHit = true;

		if(!m_pInstance_BuildHit || m_pInstance_BuildHit == Py_None){
	...
	if(m_Hit_collector == NULL){
#if USE_PYTHON_CASE_SENSIVE_ATTR
		m_Hit_collector = PyNewHitCollector(this, m_tSchema.m_dFields[m_tState.m_iField].m_sNameExactly, m_tState.m_iField);
#else
		m_Hit_collector = PyNewHitCollector(this, m_tSchema.m_dFields[m_tState.m_iField].m_sName, m_tState.m_iField);
#endif 
	}else{
		//re-init the collector
		csfHelper_HitCollectorObject *self = (csfHelper_HitCollectorObject *)m_Hit_collector;
		self->iField =  m_tState.m_iField;
#if USE_PYTHON_CASE_SENSIVE_ATTR
		*(self->m_FieldName) = m_tSchema.m_dFields[m_tState.m_iField].m_sNameExactly;
#else
		*(self->m_FieldName) = m_tSchema.m_dFields[m_tState.m_iField].m_sName;
#endif 
		//as we are in buildhit reset the position.
		self->iPhrase = 1;
	...
	if(!CheckResult(pResult))
	{
		 CSphSource_Document::BuildHits(sError, bSkipEndMarker);
	}else{
...
BYTE **	CSphSource_Python::NextDocument ( CSphString & sError ){
...

	{
		...
		if (!m_pInstance)
		{
			...
			//we do NOT care the Field
			if(m_Hit_collector == NULL)
				m_Hit_collector = PyNewHitCollector(this, m_tSchema.m_dFields[0].m_sName, 0);

			pArgs  = Py_BuildValue("(O)", m_Hit_collector);
	...
	}
	{
		...
		for(; iPrevHitPos < m_tHits.m_dData.GetLength(); iPrevHitPos++) {
			m_tHits.m_dData[iPrevHitPos].m_iDocID = m_tDocInfo.m_iDocID;
		}
			...
			while (true)
			{
				if(m_tState.m_bProcessingHits)
				{
					...
					if ( !m_tSchema.m_dFields[m_iJoinedHitField].m_bPayload && !m_tState.m_bProcessingHits && m_tHits.Length() )
						m_iJoinedHitPositions[iJoinedHitField] = HITMAN::GetPos ( m_tHits.Last()->m_iWordPos );
				...
				//create hit_c
				if(m_Hit_collector == NULL)
					m_Hit_collector = PyNewHitCollector(this, m_joinFields[m_iJoinedHitField], m_iJoinedHitField);
				else{
					csfHelper_HitCollectorObject *self = (csfHelper_HitCollectorObject *)m_Hit_collector;
					self->iField =  m_iJoinedHitField;
					*(self->m_FieldName) = m_joinFields[m_iJoinedHitField];
					self->iPhrase = 1;
					self->iPos = 1; //all pos started with 1.
				}

				pArgs  = Py_BuildValue("(sO)", m_joinFields[m_iJoinedHitField].cstr(), m_Hit_collector);
